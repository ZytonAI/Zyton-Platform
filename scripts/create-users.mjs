 ...
 *   node scripts/create-users.mjs --reset               → resetea también las contraseñas existentes
 *   node scripts/create-users.mjs --reset --only=samuel → solo esa persona
 *
...
for (const member of TEAM) {
  if (only && member.slug !== only) continue;
